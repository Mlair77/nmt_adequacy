...
class Transformer_with_lm_Decoder(DecoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
                 ...
                 train_lm, nmt_lm_loss, add_nmt_lm_loss):
        super(Transformer_with_lm_Decoder, self).__init__()
        ...
        self.lm_layers = nn.ModuleList(
            [Transformer_lm_Layer(d_model, heads, d_ff, dropout,
             attention_dropout, self_attn_type=self_attn_type,
             max_relative_positions=max_relative_positions,
             aan_useffn=aan_useffn)
             for i in range(num_layers)])
        ...
        # train language model or not
        self.train_lm = train_lm

        # add nmt-lm preds to loss ?
    ...
    def forward(self, tgt, memory_bank, step=None, generator=None, tgt_0=None, lm_embeddings=None, report_lm=False, **kwargs):
        """Decode, possibly stepwise."""
        if step == 0:
            self._init_cache(memory_bank)
        # tgt: (tgt_len, batch_size, 1)
        # memory_bank (FloatTensor): ``(src_len, batch_size, model_dim)``
        ...
        # output: batch x len x embedding_dim
        # src_memory_bank: batch x len_encoder x embedding_dim
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            # src_memory_bank (FloatTensor): ``(batch_size, src_len, model_dim)``
            # src_pad_mask (LongTensor): ``(batch_size, 1, src_len)``
            # tgt_pad_mask (LongTensor): ``(batch_size, 1, tgt_len)``
